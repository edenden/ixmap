...
#ifdef DEBUG
static void thread_packet_dump(struct ixmap_buf *buf, struct ixmap_bulk *bulk);
#endif
...
#ifdef DEBUG
				thread_packet_dump(buf, bulk_rx);
#endif

				thread_packet_forward(bulk_rx, bulk_tx_list);
				for(i = 0; i < thread->num_ports; i++){
					if(bulk > 0){
...
static void thread_packet_forward(struct ixmap_buf *buf,
